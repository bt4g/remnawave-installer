[![Версия](https://img.shields.io/badge/version-1.5.0b-blue.svg)](https://github.com/xxphantom/remnawave-installer)
...
> [!CAUTION] > **ВНИМАНИЕ!**
> Данный скрипт и репозиторий предоставляются как **образовательный пример** для изучения взаимодействия Caddy и панели Remnawave, а также для демонстрации конфигурации Caddy в качестве обратного прокси.
> **Скрипт не предназначен для использования в производственной среде без полного понимания его действий и конфигураций Remnawave.**
> Если вы не понимаете, как работает панель управления Remnawave или создаваемые скриптом конфигурации, это ваша ответственность.
...
- [Варианты защиты доступа к панели, настраиваемые скриптом](#-варианты-защиты-доступа-к-панели-настраиваемые-скриптом)
  - [SIMPLE cookie security](#simple-cookie-security)
  - [FULL Caddy security (рекомендуется)](#full-caddy-security-рекомендуется)
- [Использование скрипта: Меню и опции](#-использование-скрипта-меню-и-опции)
  - [Главное меню](#главное-меню)
  - [Меню установки](#меню-установки)
- [Важная информация, генерируемая скриптом](#-важная-информация-генерируемая-скриптом)
  - [Домены](#домены)
  - [Файл с учетными данными](#файл-с-учетными-данными)
- [Структура директорий, создаваемая скриптом](#-структура-директорий-создаваемая-скриптом)
- [Управление сервисами после установки](#-управление-сервисами-после-установки)
- [Связь с официальной установкой Remnawave](#-связь-с-официальной-установкой-remnawave)
- [Замечания и известные проблемы](#-замечания-и-известные-проблемы)
- [Remnawave: Краткий обзор компонентов](#remnawave-краткий-обзор-компонентов)
...
1.  **Установить Панель/Ноду**: Переход в меню выбора компонентов для установки.
2.  **Перезапустить панель**: Останавливает и запускает контейнеры Remnawave Panel и Subscription Page (и Caddy, если он управляется из основной директории панели).
3.  **Удалить панель**: Полностью удаляет установку Remnawave (включая Docker-вольюмы с данными!) с сервера. **Используйте с осторожностью!**
4.  **Remnawave Rescue CLI [Сброс админа]**: Запускает CLI внутри контейнера Remnawave Panel, что позволяет, например, сбросить пароль администратора панели.
5.  **Показать учетные данные панели**: Отображает содержимое файла `/opt/remnawave/credentials.txt`.
6.  **Включить/Отключить BBR**: Управляет настройками TCP BBR.
7.  **Выход**: Завершение работы скрипта.

### Меню установки

- **Только панель**:
  1.  `"FULL Caddy" вариант установки панели (рекомендуется)`
  2.  `"SIMPLE cookie" вариант установки панели`
- **Только нода**: 3. `Только нода (для отдельного сервера)`
- **All-in-One**: 4. `Панель + Нода "FULL Caddy" вариант` 5. `Панель + Нода "SIMPLE cookie" вариант`
...
Docker Hub Rate Limits: При частых установках и у некоторых хостеров вы можете столкнуться с ограничениями на скачивание образов. Скрипт сообщит об этом. Проще всего запустить docker login и авторизоваться.
