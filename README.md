[![Version](https://img.shields.io/badge/version-1.5.0b-blue.svg)](https://github.com/xxphantom/remnawave-installer)
...
- [Panel access protection options configured by the script](#-panel-access-protection-options-configured-by-the-script)
  - [SIMPLE cookie security](#simple-cookie-security)
  - [FULL Caddy security (recommended)](#full-caddy-security-recommended)
- [Using the script: Menu and options](#-using-the-script-menu-and-options)
  - [Main menu](#main-menu)
...
- [Managing services after installation](#-managing-services-after-installation)
- [Connection with official Remnawave installation](#-connection-with-official-remnawave-installation)
- [Notes and known issues](#-notes-and-known-issues)
- [Remnawave: Brief overview of components](#remnawave-brief-overview-of-components)
...
1.  **Install Panel/Node**: Go to the component selection menu for installation.
2.  **Restart panel**: Stops and starts Remnawave Panel and Subscription Page containers (and Caddy if managed from the main panel directory).
3.  **Remove panel**: Completely removes Remnawave installation (including Docker volumes with data!) from the server. **Use with caution!**
4.  **Remnawave Rescue CLI [Admin reset]**: Runs CLI inside the Remnawave Panel container, allowing you to, for example, reset the panel administrator password.
5.  **Show panel credentials**: Displays the contents of the `/opt/remnawave/credentials.txt` file.
6.  **Enable/Disable BBR**: Manages TCP BBR settings.
7.  **Exit**: Exit the script.

### Installation menu

- **Panel only**:
  1.  `"FULL Caddy" panel installation option (recommended)`
  2.  `"SIMPLE cookie" panel installation option`
- **Node only**: 3. `Node only (for separate server)`
- **All-in-One**: 4. `Panel + Node "FULL Caddy" option` 5. `Panel + Node "SIMPLE cookie" option`
- **Back to main menu**
